...
export function liveVoiceSession(config: { model?: string; voice?: string; textOnly?: boolean; turn_detection?: { threshold?: number; prefix_padding_ms?: number; silence_duration_ms?: number } | null } = {}) {
  ...
  return {
    ...
    audio: {
      input: {
        transcription: { model: OPENAI_MODELS.transcription.accurate, language: 'en' },
        noise_reduction: { type: 'near_field' },
        turn_detection: { type: 'server_vad', threshold: config.turn_detection?.threshold ?? 0.75, prefix_padding_ms: config.turn_detection?.prefix_padding_ms ?? 300, silence_duration_ms: config.turn_detection?.silence_duration_ms ?? 700, create_response: false, interrupt_response: true }
      },
