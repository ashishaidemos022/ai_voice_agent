import { BookOpen, Database, Brain, ListChecks, SlidersHorizontal } from 'lucide-react';
...
export function AnswerSourcesPanel({ sources, memory, busy = false }: { sources?: AnswerSources; memory?: MemoryReceipt; busy?: boolean }) {
  ...
  const sql = tools.filter(({ tool }) => /execute_sql|lookup_it_assets/i.test(tool.toolName));
  const policyTools = tools.filter(({ tool }) => /search_it_policy/i.test(tool.toolName));
  const adapterTools = tools.filter(({ tool }) => /query_trained_checkpoint/i.test(tool.toolName));
  const toolResult = (entries: typeof tools, empty: string) => <div className="space-y-3">{entries.map(({ tool, status }, index) => <article key={tool.id} className="rounded-lg border border-white/10 p-3"><p className="text-cyan-200">Result {index + 1} · {status} · {tool.status}</p><p className="text-xs text-white/40">{new Date(tool.createdAt).toLocaleString()}</p><pre className="mt-2 whitespace-pre-wrap break-words text-xs">{JSON.stringify(tool.response || tool.error || 'Waiting for result', null, 2)}</pre><details className="mt-2"><summary className="cursor-pointer">View request</summary><pre className="whitespace-pre-wrap break-words">{JSON.stringify(tool.request, null, 2)}</pre></details></article>)}{!entries.length && <p>{empty}</p>}</div>;
  const cards = [
    ...
    { title: 'Trained behavior', subtitle: 'Open-weight adapter · triage pattern and safety contract', icon: SlidersHorizontal, status: adapterTools.length ? `${adapterTools.filter(item => item.status === 'This turn').length} adapter calls this turn` : 'No adapter call recorded', content: toolResult(adapterTools, 'No trained behavior was supplied for this answer.') },
    {
    ...
    { title: 'Approved policy', subtitle: 'Document knowledge · retrieved through RAG', icon: BookOpen, status: policyTools.length ? `${policyTools.filter(item => item.status === 'This turn').length} policy searches this turn` : sources?.ragStatus === 'searching' ? 'Searching…' : sources?.ragStatus === 'failed' ? 'Retrieval failed' : sources?.ragStatus === 'retrieved' ? `${sources.rag?.citations.length || 0} passages retrieved this turn` : 'Not retrieved this turn', content: policyTools.length ? toolResult(policyTools, 'No policy passages supplied this turn.') : <div className="space-y-3">{sources?.rag?.citations.map((citation, index) => <article key={`${citation.file_id}-${index}`} className="rounded-lg border border-white/10 p-3"><p className="text-emerald-200">[K{index + 1}] {citation.title || citation.file_id}</p><p className="mt-2 whitespace-pre-wrap">{citation.snippet}</p></article>)}{!sources?.rag?.citations.length && <p>No document passages supplied this turn. Earlier answers may remain in the conversation.</p>}</div> },
  ...
  return <section aria-label="Sources behind this answer" className="rounded-2xl border border-cyan-300/20 bg-slate-900/60 p-5">
    ...
    <div className="mt-4 space-y-3">{cards.map(({ title, subtitle, icon: Icon, content }, index) => {
      const highlight = index === 2 ? activity.memory : index === 3 ? activity.knowledge : index === 4 ? activity.data : index === 1 ? { state: adapterTools.some(item => item.tool.status === 'succeeded') ? 'complete' as const : busy ? 'active' as const : 'idle' as const, label: adapterTools.length ? 'Trained checkpoint used' : 'Waiting for adapter' } : { state: 'idle' as const, label: sources ? 'Always included' : 'No recorded turn' };
    ...
    <p className="mt-4 text-xs text-white/40">Instructions + trained behavior + retrieved policy + live records → answer context</p>
  </section>;
}
