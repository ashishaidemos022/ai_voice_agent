...
type ModelVariant = { id: string; model: string; revision: string; precision: string; providerOnly: string[] | null; transport: 'gateway' | 'openai-compatible' | 'trained-adapter' | 'trained-fused' | 'trained-checkpoint'; runtimeModel?: string | null; supportsTools: boolean; datasetName?: string; trainedAt?: number };
type TrainingJob = { id: string; backend?: 'local' | 'tinker'; name: string; dataset_name: string; status: string; artifact_sha256?: string; completed_at?: number; promotion_status?: string; fused_sha256?: string; promoted_checkpoint_path?: string };
  ...
  useEffect(() => {
    ...
    fetch('/api/open-weight-chat', { headers })
      ...
      .then((registry) => {
        ...
        setModels((current) => [
          ...registered,
          ...current.filter((model) => model.transport === 'trained-adapter' || model.transport === 'trained-fused' || model.transport === 'trained-checkpoint'),
        ]);
  ...
  const runComparison = async () => {
    ...
    const next = await Promise.all(selectedModels.map(async (modelId): Promise<LabResult> => {
      try {
        ...
        const isTrained = model?.transport === 'trained-adapter' || model?.transport === 'trained-fused' || model?.transport === 'trained-checkpoint';
        const response = await fetch(isTrained ? `/api/open-weight-training?jobId=${encodeURIComponent(model.runtimeModel || '')}&action=${model.transport === 'trained-fused' ? 'fused-completion' : 'completion'}` : '/api/open-weight-chat', {
  ...
  return <MainLayout sidebar={sidebar} topBar={topBar}><div className="h-full overflow-y-auto px-5 py-6"><div className="mx-auto max-w-6xl space-y-5">
    ...
    {view === 'run' && <>
      ...
      <Card className="space-y-5 border-white/10 bg-white/[0.03] p-5">
        ...
        <div className="grid gap-4 md:grid-cols-[1fr_1fr_2fr]"><label className="text-xs text-white/45">Temperature <span className="float-right text-white/70">{temperature.toFixed(1)}</span><input aria-label="Temperature" type="range" min="0" max="1" step="0.1" value={temperature} onChange={(e) => setTemperature(Number(e.target.value))} className="mt-3 w-full accent-amber-400" /></label><label className="text-xs text-white/45">Maximum output<select value={maxTokens} onChange={(e) => setMaxTokens(Number(e.target.value))} className="mt-2 w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-xs text-white"><option value="128">128 tokens</option><option value="256">256 tokens</option><option value="512">512 tokens</option><option value="1024">1,024 tokens</option></select></label><div><div className="flex items-center justify-between gap-3"><p className="text-xs text-white/45">Models</p>{controlledModelIds.length > 0 && <button type="button" onClick={() => setSelectedModels(controlledModelIds)} className="text-[11px] text-amber-200/70 hover:text-amber-100">Select legacy Aster trio</button>}</div><div className="mt-2 flex flex-wrap gap-2">{models.map((model) => { const trained = model.transport === 'trained-adapter' || model.transport === 'trained-fused' || model.transport === 'trained-checkpoint'; return <label key={model.id} title={model.model} className={`flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-xs ${trained ? 'border-emerald-300/25 bg-emerald-400/[0.05] text-emerald-100' : 'border-white/10 text-white/60'}`}><input type="checkbox" checked={selectedModels.includes(model.id)} onChange={() => setSelectedModels((current) => current.includes(model.id) ? current.filter((id) => id !== model.id) : [...current, model.id])} className="accent-amber-400" /><span>{trained ? model.model.split(' · ')[0] : model.id}</span>{trained && <span className="rounded bg-emerald-400/10 px-1.5 py-0.5 text-[9px] uppercase text-emerald-200/70">{model.transport === 'trained-fused' ? 'fused' : model.transport === 'trained-checkpoint' ? 'promoted' : 'trained'}</span>}</label>; })}</div>{models.some((model) => model.transport === 'trained-adapter') && <p className="mt-2 text-[10px] text-white/35">Completed training jobs are green. Qwen promotions expose fused weights; Inkling promotions preserve the evaluated Tinker checkpoint.</p>}</div></div>
    ...
    {view === 'models' && <div className="space-y-4"><Card className="border-white/10 bg-white/[0.03] p-5"><div className="flex items-center gap-2"><Settings2 className="h-4 w-4 text-amber-200" /><h3 className="font-semibold text-white">Endpoint and adapter registry</h3></div><p className="mt-2 text-sm text-white/45">Registered checkpoints and every completed training job can receive live requests. Each trained adapter remains tied to its dataset hash and immutable artifact hash.</p></Card><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{models.map((model) => { const trained = model.transport === 'trained-adapter' || model.transport === 'trained-fused' || model.transport === 'trained-checkpoint'; return <Card key={model.id} className="border-emerald-300/15 bg-emerald-400/[0.04] p-5"><div className="flex items-center justify-between"><Layers3 className="h-5 w-5 text-emerald-200" /><span className="rounded-full bg-emerald-400/10 px-2 py-1 text-[10px] uppercase text-emerald-200">{model.transport === 'trained-fused' || model.transport === 'trained-checkpoint' ? 'Promoted' : trained ? 'Trained job' : 'Live'}</span></div><h4 className="mt-4 font-semibold text-white">{trained ? model.model.split(' · ')[0] : model.id}</h4><p className="mt-1 text-xs text-white/45">{trained ? `${model.datasetName} · ${model.transport === 'trained-fused' ? 'fused checkpoint' : model.transport === 'trained-checkpoint' ? 'Tinker checkpoint' : 'LoRA adapter'} · ${model.runtimeModel}` : model.model}</p><dl className="mt-4 grid grid-cols-2 gap-3 text-xs"><div><dt className="text-white/30">Revision</dt><dd className="mt-1 break-all text-white/60">{model.revision}</dd></div><div><dt className="text-white/30">Precision</dt><dd className="mt-1 text-white/60">{model.precision}</dd></div><div><dt className="text-white/30">Runtime</dt><dd className="mt-1 text-white/60">{model.transport === 'gateway' ? 'Vercel Gateway' : model.transport === 'trained-checkpoint' ? 'Tinker' : 'Viaana GPU'}</dd></div><div><dt className="text-white/30">Tools</dt><dd className="mt-1 text-white/60">{model.supportsTools ? 'Supported' : 'Prompt only'}</dd></div></dl></Card>; })}</div></div>}
  </div></div></MainLayout>;
}
