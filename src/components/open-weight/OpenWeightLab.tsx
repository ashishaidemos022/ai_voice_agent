...
type ModelVariant = { id: string; model: string; revision: string; precision: string; providerOnly: string[] | null; transport: 'gateway' | 'openai-compatible'; runtimeModel?: string | null; supportsTools: boolean };
...
export function OpenWeightLab(props: OpenWeightLabProps) {
  ...
  return <MainLayout sidebar={sidebar} topBar={topBar}><div className="h-full overflow-y-auto px-5 py-6"><div className="mx-auto max-w-6xl space-y-5">
    ...
    {view === 'models' && <div className="space-y-4"><Card className="border-white/10 bg-white/[0.03] p-5"><div className="flex items-center gap-2"><Settings2 className="h-4 w-4 text-amber-200" /><h3 className="font-semibold text-white">Endpoint registry</h3></div><p className="mt-2 text-sm text-white/45">Only server-approved model IDs can receive live requests. The controlled variants share one GPU runtime while loading separate base, adapter, and fused weights.</p></Card><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{models.map((model) => <Card key={model.id} className="border-emerald-300/15 bg-emerald-400/[0.04] p-5"><div className="flex items-center justify-between"><Layers3 className="h-5 w-5 text-emerald-200" /><span className="rounded-full bg-emerald-400/10 px-2 py-1 text-[10px] uppercase text-emerald-200">Live</span></div><h4 className="mt-4 font-semibold text-white">{model.id}</h4><p className="mt-1 text-xs text-white/45">{model.model}</p><dl className="mt-4 grid grid-cols-2 gap-3 text-xs"><div><dt className="text-white/30">Revision</dt><dd className="mt-1 break-all text-white/60">{model.revision}</dd></div><div><dt className="text-white/30">Precision</dt><dd className="mt-1 text-white/60">{model.precision}</dd></div><div><dt className="text-white/30">Runtime</dt><dd className="mt-1 text-white/60">{model.transport === 'gateway' ? 'Vercel Gateway' : 'Viaana GPU'}</dd></div><div><dt className="text-white/30">Tools</dt><dd className="mt-1 text-white/60">{model.supportsTools ? 'Supported' : 'Prompt only'}</dd></div></dl></Card>)}</div></div>}
  </div></div></MainLayout>;
}
