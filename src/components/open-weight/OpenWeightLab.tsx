...
type ModelVariant = { id: string; model: string; revision: string; precision: string; providerOnly: string[] | null; transport: 'gateway' | 'openai-compatible' | 'trained-adapter' | 'trained-fused'; runtimeModel?: string | null; supportsTools: boolean; datasetName?: string; trainedAt?: number };
type TrainingJob = { id: string; name: string; dataset_name: string; status: string; artifact_sha256?: string; completed_at?: number; promotion_status?: string; fused_sha256?: string };
...
export function OpenWeightLab(props: OpenWeightLabProps) {
  ...
  const runComparison = async () => {
    ...
    const next = await Promise.all(selectedModels.map(async (modelId): Promise<LabResult> => {
      try {
        ...
        const isTrained = model?.transport === 'trained-adapter' || model?.transport === 'trained-fused';
  ...
  return <MainLayout sidebar={sidebar} topBar={topBar}><div className="h-full overflow-y-auto px-5 py-6"><div className="mx-auto max-w-6xl space-y-5">
    ...
    {view === 'models' && <div className="space-y-4"><Card className="border-white/10 bg-white/[0.03] p-5"><div className="flex items-center gap-2"><Settings2 className="h-4 w-4 text-amber-200" /><h3 className="font-semibold text-white">Endpoint and adapter registry</h3></div><p className="mt-2 text-sm text-white/45">Registered checkpoints and every completed training job can receive live requests. Each trained adapter remains tied to its dataset hash and immutable artifact hash.</p></Card><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{models.map((model) => { const trained = model.transport === 'trained-adapter' || model.transport === 'trained-fused'; return <Card key={model.id} className="border-emerald-300/15 bg-emerald-400/[0.04] p-5"><div className="flex items-center justify-between"><Layers3 className="h-5 w-5 text-emerald-200" /><span className="rounded-full bg-emerald-400/10 px-2 py-1 text-[10px] uppercase text-emerald-200">{model.transport === 'trained-fused' ? 'Promoted' : trained ? 'Trained job' : 'Live'}</span></div><h4 className="mt-4 font-semibold text-white">{trained ? model.model.split(' · ')[0] : model.id}</h4><p className="mt-1 text-xs text-white/45">{trained ? `${model.datasetName} · ${model.transport === 'trained-fused' ? 'fused checkpoint' : 'LoRA adapter'} · ${model.runtimeModel}` : model.model}</p><dl className="mt-4 grid grid-cols-2 gap-3 text-xs"><div><dt className="text-white/30">Revision</dt><dd className="mt-1 break-all text-white/60">{model.revision}</dd></div><div><dt className="text-white/30">Precision</dt><dd className="mt-1 text-white/60">{model.precision}</dd></div><div><dt className="text-white/30">Runtime</dt><dd className="mt-1 text-white/60">{model.transport === 'gateway' ? 'Vercel Gateway' : 'Viaana GPU'}</dd></div><div><dt className="text-white/30">Tools</dt><dd className="mt-1 text-white/60">{model.supportsTools ? 'Supported' : 'Prompt only'}</dd></div></dl></Card>; })}</div></div>}
  </div></div></MainLayout>;
}
