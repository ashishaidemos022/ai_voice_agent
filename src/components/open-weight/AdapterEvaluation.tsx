...
const wrenCases: AdapterTestCase[] = [
  ...
  { id: 'gift-card-cross-location', category: 'gift-cards', messages: [{ role: 'system', content: WREN_SYSTEM }, { role: 'user', content: 'Can I purchase a $75 Wren gift card online and use it at a different Wren location?' }], expected: { required: [['online'], ['$75', '75'], ['any Wren location', 'different Wren location', 'all Wren locations']] } },
];
...
export function AdapterEvaluation({ accessToken, baseModelId, job, onJobUpdated }: { accessToken: string; baseModelId?: string; job: Job; onJobUpdated: (job: Job) => void }) {
  ...
  useEffect(() => { setRun(null); setError(''); setJsonl(/wren/i.test(`${job.name} ${job.dataset_name}`) ? wrenJsonl : ''); }, [job.id]);
  ...
  const promote = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/open-weight-training?jobId=${encodeURIComponent(job.id)}&action=promote`, {
        method: 'POST', headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ evaluation_id: run.id, evaluation_sha256: await sha256(run), adapter_passed: adapterPassed, base_passed: basePassed, total: run.cases.length }),
      });
      const body = await readJson(response);
  ...
  const canPromote = !!run && adapterPassed === run.cases.length && adapterPassed > basePassed && !run.cases.some((item) => item.adapter.error);
