...
type Job = {
  ...
  backend?: 'local' | 'tinker'; promotion_status?: string; promoted_at?: number; fused_sha256?: string; fused_repository_path?: string;
  promoted_checkpoint_path?: string; promotion_kind?: string;
};
...
const wrenCases: AdapterTestCase[] = [
  { id: 'reservations-weekend', category: 'reservations', messages: [{ role: 'system', content: WREN_ADAPTER_SYSTEM_PROMPT }, { role: 'user', content: 'Should I book ahead for a Saturday evening, or can I walk in?' }], expected: { required: [['recommended'], ['dinner'], ['weekend', 'Saturday'], ['online', 'phone'], ['walk-in', 'walk in']] } },
  { id: 'dietary-allergy', category: 'dietary', messages: [{ role: 'system', content: WREN_ADAPTER_SYSTEM_PROMPT }, { role: 'user', content: 'One guest is vegan and another has an allergy. Can Wren help?' }], expected: { required: [['vegan'], ['vegetarian'], ['gluten-free', 'gluten free'], ['server'], ['allerg', 'kitchen']] } },
  { id: 'large-party-nine', category: 'groups', messages: [{ role: 'system', content: WREN_ADAPTER_SYSTEM_PROMPT }, { role: 'user', content: 'What should I do if I want to bring a group of nine?' }], expected: { required: [['eight or more', '8 or more'], ['contact'], ['advance'], ['set menu', 'private space']], allowedNumbers: ['8', '9'] } },
  { id: 'delivery-location', category: 'takeout', messages: [{ role: 'system', content: WREN_ADAPTER_SYSTEM_PROMPT }, { role: 'user', content: 'Can every Wren location deliver my order?' }], expected: { required: [['Most', 'most'], ['takeout'], ['selected areas'], ['delivery partners'], ['website', 'location page']] } },
  { id: 'accessibility-needs', category: 'accessibility', messages: [{ role: 'system', content: WREN_ADAPTER_SYSTEM_PROMPT }, { role: 'user', content: 'Are the dining room and restroom accessible, and can I request help in advance?' }], expected: { required: [['dining rooms', 'dining room'], ['restrooms', 'restroom'], ['accessible'], ['booking', 'book']] } },
  { id: 'corkage-policy', category: 'corkage', messages: [{ role: 'system', content: WREN_ADAPTER_SYSTEM_PROMPT }, { role: 'user', content: 'Is outside wine allowed, and exactly how much is corkage?' }], expected: { required: [['some locations', 'selected locations'], ['small fee'], ['call ahead', 'confirm']], allowedNumbers: [] } },
  { id: 'hours-saturday', category: 'hours', messages: [{ role: 'system', content: WREN_ADAPTER_SYSTEM_PROMPT }, { role: 'user', content: 'Does Wren serve lunch on Saturdays, and what time does it end?' }], expected: { required: [['11:30 a.m.', '11:30 am'], ['2:30 p.m.', '2:30 pm'], ['Tuesday–Sunday', 'Tuesday-Sunday', 'Tuesday through Sunday', 'Tuesday to Sunday'], ['location', 'locations'], ['holiday', 'holidays']], forbidden: ['Monday'], allowedNumbers: ['11:30', '2:30', '5:00', '10:00'] } },
  { id: 'children-family', category: 'children', messages: [{ role: 'system', content: WREN_ADAPTER_SYSTEM_PROMPT }, { role: 'user', content: 'What does Wren provide for a family dining with young children?' }], expected: { required: [["children's menu", 'childrens menu'], ['high chairs', 'highchairs'], ['booster seats', 'booster'], ['lunch', 'early dinner']] } },
  { id: 'dress-formality', category: 'dress-code', messages: [{ role: 'system', content: WREN_ADAPTER_SYSTEM_PROMPT }, { role: 'user', content: 'Do I need to dress formally for dinner at Wren?' }], expected: { required: [['smart-casual', 'smart casual'], ['no formal', 'not formal'], ['relaxed', 'welcoming']] } },
  { id: 'parking-options', category: 'parking', messages: [{ role: 'system', content: WREN_ADAPTER_SYSTEM_PROMPT }, { role: 'user', content: 'Will there be a Wren parking lot when I arrive?' }], expected: { required: [['vary by location', 'varies by location'], ['lot'], ['street parking'], ['public garages', 'public garage'], ['location page', "location's page"]] } },
  ...
  { id: 'feedback-contact', category: 'feedback', messages: [{ role: 'system', content: WREN_ADAPTER_SYSTEM_PROMPT }, { role: 'user', content: 'How can I send comments about a recent visit?' }], expected: { required: [['phone'], ['email'], ['location'], ['manager']] } },
  { id: 'unknown-breakfast', category: 'unknown', messages: [{ role: 'system', content: WREN_ADAPTER_SYSTEM_PROMPT }, { role: 'user', content: 'What time does breakfast begin?' }], expected: { required: [["don't have approved information", 'do not have approved information', "don't have information", 'not provided'], ['local Wren', 'location']], allowedNumbers: [] } },
];
...
export function AdapterEvaluation({ accessToken, baseModelId, job, onJobUpdated }: { accessToken: string; baseModelId?: string; job: Job; onJobUpdated: (job: Job) => void }) {
  ...
  const supportsFused = job.backend !== 'tinker';
  const evaluate = async (includeFused = job.promotion_status === 'promoted' && supportsFused) => {
  ...
  return <Card className="space-y-5 border-violet-300/15 bg-violet-400/[0.03] p-5">
    ...
    {job.promotion_status === 'promoted' && <div className="rounded-xl border border-emerald-300/20 bg-emerald-400/[0.05] p-3 text-xs text-emerald-100"><p className="font-semibold">Promoted {supportsFused ? 'fused checkpoint' : 'Tinker checkpoint'}</p><p className="mt-1 break-all font-mono text-[10px] text-emerald-200/60">{supportsFused ? `SHA-256 ${job.fused_sha256} · ${job.fused_repository_path}` : job.promoted_checkpoint_path}</p></div>}
