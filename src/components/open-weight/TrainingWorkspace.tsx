...
type Job = {
  ...
  examples?: Example[];
  backend?: 'local' | 'tinker'; base_model?: string; tinker_run_url?: string; checkpoint_path?: string; sampler_checkpoint_path?: string; artifact_hash_kind?: string;
...
export function TrainingWorkspace({ accessToken, localBaseModelId, tinkerBaseModelId, onJobDeleted }: Props) {
  ...
  const launch = async () => {
    ...
    try {
      const body = await readJson(await fetch('/api/open-weight-training', { method: 'POST', headers, body: JSON.stringify({ backend, ...(backend === 'tinker' ? { base_model: 'thinkingmachines/Inkling-Small' } : {}), name: jobName, dataset_name: datasetName, examples: parsed.examples, rank, alpha, learning_rate: learningRate, max_steps: maxSteps, seed }) }));
  ...
  const selectJob = async (job: Job) => {
    setActiveJob(job); setLoadingJobId(job.id); setError(''); setComparison({});
    try {
      const body = await readJson(await fetch(`/api/open-weight-training?jobId=${encodeURIComponent(job.id)}`, { headers }));
      const selected = body.job as Job;
      if (!Array.isArray(selected.examples) || !selected.examples.length) throw new Error('This older job does not have a saved dataset snapshot');
      setActiveJob(selected);
      setJobs((current) => current.map((item) => item.id === selected.id ? { ...item, ...selected } : item));
      setBackend(selected.backend === 'tinker' ? 'tinker' : 'local');
      setDatasetName(selected.dataset_name);
      setJobName(selected.name);
      setJsonl(selected.examples.map((example) => JSON.stringify(example)).join('\n'));
      setRank(selected.rank);
      setAlpha(selected.alpha);
      setLearningRate(selected.learning_rate);
      setMaxSteps(selected.max_steps);
      setSeed(selected.seed);
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to load the training dataset'); }
    setLoadingJobId('');
  };

  const compare = async () => {
    if (!activeJob || activeJob.status !== 'completed' || !activeBaseModelId) return;
    ...
    try {
      const [base, trained] = await Promise.all([
        readJson(await fetch('/api/open-weight-chat', { method: 'POST', headers, body: JSON.stringify({ modelId: activeBaseModelId, messages, temperature: 0, maxTokens: 128 }) })),
  ...
  return <div className="space-y-5">
    <div className="grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
      ...
      <div className="space-y-4">
        <Card className="space-y-4 border-white/10 bg-white/[0.03] p-5"><div className="flex items-center gap-2"><Weight className="h-4 w-4 text-amber-200" /><h3 className="font-semibold text-white">LoRA configuration</h3></div>
          <label className="block text-xs text-white/45">Training backend<select value={backend} onChange={(event) => setBackend(event.target.value as 'local' | 'tinker')} className="mt-2 w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-white"><option value="tinker">Tinker · Inkling-Small</option><option value="local">Private T4 · Qwen3 0.6B</option></select></label>
          <div className="rounded-xl border border-white/10 bg-slate-950/50 p-3"><p className="text-xs font-medium text-white/65">{backend === 'tinker' ? 'thinkingmachines/Inkling-Small' : 'bhatsy/viaana-qwen3-0.6b-base'}</p><p className="mt-1 text-[11px] leading-5 text-white/35">{backend === 'tinker' ? 'Remote LoRA updates run on Tinker. The saved tinker:// checkpoint is used for sampling and promotion.' : 'LoRA updates run on the private Hugging Face T4 and can be fused into the local base.'}</p></div>
          <div className="grid grid-cols-2 gap-3"><label className="text-xs text-white/45">Rank<select value={rank} onChange={(e) => setRank(Number(e.target.value))} className="mt-2 w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-white"><option>4</option><option>8</option><option>16</option></select></label><label className="text-xs text-white/45">Alpha {backend === 'tinker' && <span className="text-white/25">(Qwen only)</span>}<input disabled={backend === 'tinker'} type="number" min="4" max="64" value={alpha} onChange={(e) => setAlpha(Number(e.target.value))} className="mt-2 w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-white disabled:cursor-not-allowed disabled:opacity-35" /></label><label className="text-xs text-white/45">Learning rate<input type="number" min="0.00001" max="0.002" step="0.00001" value={learningRate} onChange={(e) => setLearningRate(Number(e.target.value))} className="mt-2 w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-white" /></label><label className="text-xs text-white/45">Steps<input type="number" min="2" max="200" value={maxSteps} onChange={(e) => setMaxSteps(Number(e.target.value))} className="mt-2 w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-white" /><span className="mt-1 block text-[10px] text-white/30">For factual adapters, use enough steps to cover the dataset at least twice.</span></label><label className="col-span-2 text-xs text-white/45">Seed<input type="number" min="0" value={seed} onChange={(e) => setSeed(Number(e.target.value))} className="mt-2 w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-white" /></label></div>
          <Button onClick={launch} disabled={busy || !!parsed.error} className="w-full bg-amber-400 text-slate-950 hover:bg-amber-300">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" />} Train {backend === 'tinker' ? 'Inkling adapter' : 'adapter'}</Button><p className="text-[11px] leading-5 text-white/35">One job per backend can train at a time; inputs are capped at 200 examples and 200 steps.</p></Card>
        {activeJob && <Card className="border-amber-300/15 bg-amber-400/[0.04] p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] uppercase tracking-wider text-white/35">Current job · {activeJob.backend === 'tinker' ? 'Tinker / Inkling-Small' : 'Private T4 / Qwen'}</p><h4 className="mt-1 text-sm font-semibold text-white">{activeJob.name}</h4><p className="mt-1 font-mono text-[10px] text-white/35">{activeJob.id}</p></div>{activeJob.status === 'completed' ? <CheckCircle2 className="h-5 w-5 text-emerald-300" /> : activeJob.status === 'failed' ? <span className="text-xs text-rose-300">Failed</span> : <Loader2 className="h-5 w-5 animate-spin text-amber-200" />}</div><div className="mt-4 h-2 overflow-hidden rounded-full bg-white/5"><div className="h-full bg-amber-300 transition-all" style={{ width: `${activeJob.progress || 0}%` }} /></div><div className="mt-3 flex justify-between text-xs text-white/45"><span className="capitalize">{activeJob.status}</span><span>{activeJob.step || 0}/{activeJob.max_steps} steps{activeJob.loss == null ? '' : ` · loss ${activeJob.loss}`}</span></div>{activeJob.tinker_run_url && <a className="mt-3 block text-xs text-amber-200/70 hover:text-amber-100" href={activeJob.tinker_run_url} target="_blank" rel="noreferrer">Open live Tinker run ↗</a>}{activeJob.error && <p className="mt-3 text-xs text-rose-300">{activeJob.error}</p>}{activeJob.artifact_sha256 && <div className="mt-4 space-y-1 border-t border-white/10 pt-3 text-[11px] text-white/40"><p>{activeJob.artifact_hash_kind === 'checkpoint-provenance' ? 'Checkpoint provenance' : 'Adapter'} SHA-256 <span className="font-mono text-emerald-200/70">{activeJob.artifact_sha256}</span></p><p className="break-all">{activeJob.repository}/{activeJob.repository_path}</p><p>Loss {activeJob.initial_loss} → {activeJob.final_loss}{activeJob.trainable_parameters ? ` · ${activeJob.trainable_parameters.toLocaleString()} trainable parameters` : ''}</p></div>}</Card>}
      </div>
    </div>
    ...
    {!!jobs.length && <Card className="overflow-hidden border-white/10 bg-white/[0.03]"><div className="flex items-center gap-2 border-b border-white/10 p-4"><FileJson className="h-4 w-4 text-white/45" /><div><h3 className="text-sm font-semibold text-white">Adapter registry</h3><p className="mt-1 text-[11px] text-white/35">Select a job to restore its exact dataset and training configuration. Deleting it permanently removes all artifacts and evidence.</p></div></div><div className="divide-y divide-white/5">{jobs.map((job) => <div key={job.id} className="flex items-center gap-2 pr-3 hover:bg-white/[0.03]"><button onClick={() => selectJob(job)} disabled={loadingJobId === job.id} className="flex min-w-0 flex-1 items-center gap-4 p-4 text-left disabled:opacity-60"><span className={`h-2 w-2 shrink-0 rounded-full ${job.status === 'completed' ? 'bg-emerald-400' : job.status === 'failed' ? 'bg-rose-400' : 'bg-amber-300'}`} /><div className="min-w-0 flex-1"><p className="truncate text-sm text-white/70">{job.name}</p><p className="mt-1 truncate text-[11px] text-white/35">{job.backend === 'tinker' ? 'Inkling-Small · Tinker' : 'Qwen3 0.6B · private T4'} · {job.dataset_name} · {job.example_count} examples · rank {job.rank} · {job.max_steps} steps{job.fused_sha256 || job.promoted_checkpoint_path ? ' · promoted' : ''}</p></div>{loadingJobId === job.id ? <Loader2 className="h-4 w-4 animate-spin text-amber-200" /> : <span className="text-xs capitalize text-white/40">{job.status}</span>}</button><button type="button" onClick={() => deleteJob(job)} disabled={deletingJobId === job.id || ['queued', 'loading', 'training', 'saving'].includes(job.status)} className="rounded-lg border border-rose-300/15 p-2 text-rose-200/55 transition hover:border-rose-300/30 hover:bg-rose-400/10 hover:text-rose-100 disabled:cursor-not-allowed disabled:opacity-25" aria-label={`Delete ${job.name}`} title="Delete adapter and all related artifacts">{deletingJobId === job.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}</button></div>)}</div></Card>}
