    ...
    <div className="grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
      ...
      <div className="space-y-4">
        <Card className="space-y-4 border-white/10 bg-white/[0.03] p-5"><div className="flex items-center gap-2"><Weight className="h-4 w-4 text-amber-200" /><h3 className="font-semibold text-white">LoRA configuration</h3></div>
          ...
          <div className="grid grid-cols-2 gap-3"><label className="text-xs text-white/45">Rank<select value={rank} onChange={(e) => setRank(Number(e.target.value))} className="mt-2 w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-white"><option>4</option><option>8</option><option>16</option></select></label><label className="text-xs text-white/45">Alpha {backend === 'tinker' && <span className="text-white/25">(Qwen only)</span>}<input disabled={backend === 'tinker'} type="number" min="4" max="64" value={alpha} onChange={(e) => setAlpha(Number(e.target.value))} className="mt-2 w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-white disabled:cursor-not-allowed disabled:opacity-35" /></label><label className="text-xs text-white/45">Learning rate<input type="number" min="0.00001" max="0.002" step="0.00001" value={learningRate} onChange={(e) => setLearningRate(Number(e.target.value))} className="mt-2 w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-white" /></label><label className="text-xs text-white/45">Steps<input type="number" min="2" max="200" value={maxSteps} onChange={(e) => setMaxSteps(Number(e.target.value))} className="mt-2 w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-white" /></label><label className="col-span-2 text-xs text-white/45">Seed<input type="number" min="0" value={seed} onChange={(e) => setSeed(Number(e.target.value))} className="mt-2 w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-white" /></label></div>
