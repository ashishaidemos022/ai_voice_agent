...
async function runRagQuery(params: {
  ...
  const guardrailViolated =
    params.ragMode === 'guardrail' && (!citations.length || answerFromModel?.toUpperCase?.() === 'INSUFFICIENT');
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const tenantId: string = vaUser.id;
    ...
    if (action === 'run_query') {
      ...
      if (!spaces || spaces.some((space: any) => space.tenant_id !== tenantId)) {
        return new Response(JSON.stringify({ error: 'Invalid knowledge space selection' }), {
          status: 403,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        });
      }
      const vectorStoreIds = spaces
